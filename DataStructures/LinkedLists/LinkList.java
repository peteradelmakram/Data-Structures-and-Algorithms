...
public class LinkList {
  ...
  // Pushes even values in the linked list so that they appear after odd values
  // in the list.
  ...
  public void deleteAllExceptRange(int n, int m) {
    Link curr = head;
    for (int i = 1; i < n; i++) {
      curr = curr.next;
    }
  ...
  public void swapper(int n, int m) {
    int c = 0, d = 0;
    Link preN = head, preM = head;
    ...
    Link currM = preM.next;
    Link currN = preN.next;
    Link afterN = currN.next;
    Link afterM = currM.next;
    preN.next = currM;
    currM.next = afterN;
    preM.next = currN;
    currN.next = afterM;

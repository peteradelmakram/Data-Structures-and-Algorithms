public class MaxHeap {
    ...
    /*            
                            10 at index == 1 (node.left at index 2*1 && node.right at 2*1+1)
                          /    \    
                        9       8 left at index 2 and right at index 3. (for left: left at 2*2 && right at 2*2+1) & (for right: left at 2*3 && right at 2*3+1)
                     /    \    /   \
                     7(4) 6(5) 5(6) 4(7)
                   /   \ / \ / \   / \
    ...
    int[] sampleHeap = {Integer.MIN_VALUE, 16,10,4,5,6,14,7,3,2,1,4,2,8,1};
    ...
    public void insert(int x){
        ...
        if(size == heap.length - 1) heap = resize(heap.length + 1);
    ...
    public void sink(int k){
        //There must be enough parents to compare with children, so 2*k must be less than or equal to n (total number of nodes.)
        while(2*k <= size){
            //the left child of k.
            int j = 2*k; // == (k.left)
            //Must ensure that parent has a right child aswell && if the right child is greater than the left child, go to right and increment j.
